  ...
  return {
    ...
    async set(key, arr) {
      if (key === 'reviews:approved' || key === 'reviews:pending') {
        ...
        // Upsert by id so a review moving pending<->approved just flips its
        // status. (A plain delete-then-insert would hit a primary-key conflict
        // because the same id can briefly exist under the old status.)
        if (list.length) {
          const rows = list.map((r) => ({
            id: r.id, name: r.name, rating: r.rating, text: r.text,
            status, created_at: toIso(r.ts),
          }));
          const { error: upErr } = await db.from('reviews').upsert(rows, { onConflict: 'id' });
          if (upErr) throw upErr;
        }
        // Drop rows of THIS status that are no longer in the array.
        const ids = list.map((r) => r.id);
        let del = db.from('reviews').delete().eq('status', status);
